...
const PublicLayout = () => {
  ...
  return (
    <div className={`flex flex-col min-h-screen font-sans bg-slate-950 ${user ? 'md:flex-row' : ''}`}>
      
      {user ? (
        <>
          <PublicSidebar mobileMenuOpen={mobileMenuOpen} setMobileMenuOpen={setMobileMenuOpen} />
          {/* Mobile Header for Sidebar Mode */}
          <div className="md:hidden flex items-center justify-between h-16 bg-slate-950/80 backdrop-blur-md px-4 border-b border-slate-800 sticky top-0 z-30">
            <Link to="/inquiry" className="flex items-center cursor-pointer">
               <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center mr-2 shadow-[0_0_15px_rgba(37,99,235,0.5)]">
                 <span className="text-white font-bold text-lg">S</span>
               </div>
               <span className="font-bold text-xl text-white tracking-tight">SupportFlow</span>
            </Link>
            <button onClick={() => setMobileMenuOpen(true)} className="text-slate-400 hover:text-white transition-colors">
               <Menu size={24} />
            </button>
          </div>
        </>
      ...
      <div className={`flex-grow flex flex-col w-full ${user ? 'md:pl-64' : ''}`}>
        ...
        {/* Simple Dark Footer */}
        {!user && (
          <footer className="bg-slate-950 py-8 border-t border-slate-900 mt-auto">
