      ...
      <div className="max-w-2xl w-full bg-slate-900/60 backdrop-blur-xl p-6 sm:p-8 rounded-3xl shadow-lg border border-slate-900 dark:border-white/50">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea required rows={5} value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-2 bg-slate-900/50 border border-slate-900 dark:border-white/60 backdrop-blur-sm rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none transition-all resize-none shadow-inner" placeholder="Provide as much detail as possible..." />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Priority (Optional)</label>
            <select value={formData.priority} onChange={(e) => setFormData({...formData, priority: e.target.value})} className="w-full px-4 py-2 bg-slate-900/50 border border-slate-900 dark:border-white/60 backdrop-blur-sm rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none transition-all shadow-inner">
