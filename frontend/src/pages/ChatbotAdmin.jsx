  ...
  return (
    <div className="h-full flex flex-col max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Chatbot Conversations</h2>
          <p className="text-slate-400 mt-2">Review transcripts and convert interactions into tickets or leads.</p>
        </div>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        {/* Sidebar List */}
        <div className="w-1/3 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm overflow-hidden flex flex-col">
          <div className="p-5 border-b border-slate-800 bg-slate-950/50">
            <h3 className="font-extrabold text-slate-100 mb-3 text-lg">Session History</h3>
            <div className="relative">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
              <input 
                type="text" 
                placeholder="Search sessions..." 
                className="w-full pl-9 pr-3 py-2 bg-slate-900 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none shadow-sm transition text-sm"
                value={searchQuery}
            ...
            {conversations.length === 0 ? (
              ...
              conversations
                ...
                <div 
                  ...
                  className={`p-5 cursor-pointer transition-all ${selectedConv?._id === conv._id ? 'bg-fuchsia-50/50 border-l-4 border-fuchsia-500' : 'border-l-4 border-transparent hover:bg-slate-950'}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-sm font-bold truncate pr-2 ${selectedConv?._id === conv._id ? 'text-fuchsia-900' : 'text-slate-100'}`}>Session: {conv.sessionId.substring(0, 15)}...</span>
                  </div>
                  ...
                  <div className="text-[11px] font-bold text-slate-300 flex items-center bg-slate-800/80 px-2.5 py-1 rounded-md w-max border border-slate-800/60 uppercase tracking-wider">
        ...
        {/* Chat Detail View */}
        <div className="flex-1 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex flex-col overflow-hidden">
          {selectedConv ? (
            <>
              <div className="p-6 border-b border-slate-800 bg-slate-900 flex flex-col md:flex-row md:items-center justify-between gap-4 z-10 shadow-sm">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-extrabold text-white text-lg tracking-tight">AI Chat Transcript</h3>
                ...
                <div className="flex space-x-3 items-center">
                  <div className="flex space-x-3">
                    <button onClick={handleConvertToTicket} className="text-sm font-bold bg-slate-900 border-2 border-slate-800 text-slate-200 px-4 py-2 rounded-xl hover:bg-slate-950 hover:border-slate-700 transition flex items-center group">
                      <Plus size={16} className="mr-2 text-blue-500 group-hover:scale-110 transition-transform" />
                      Create Ticket
                    </button>
                    <button onClick={handleConvertToLead} className="text-sm font-bold bg-slate-900 border-2 border-slate-800 text-slate-200 px-4 py-2 rounded-xl hover:bg-slate-950 hover:border-slate-700 transition flex items-center group">
              ...
              <div className="flex-1 p-8 overflow-y-auto space-y-6 bg-slate-950/80">
                <div className="text-center mb-8">
                   <span className="bg-slate-200 text-slate-300 text-xs font-bold px-3 py-1 rounded-full">
                ...
                {selectedConv.messages.map((msg, idx) => (
                  <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`flex items-center mb-2 space-x-2 px-1 ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                      {msg.role === 'user' ? (
                        <>
                           <div className="h-6 w-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-400"><User size={12}/></div>
                           <span className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">User</span>
                        </>
                      ) : (
                        <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-950/50">
              <div className="h-24 w-24 bg-slate-900 rounded-full flex items-center justify-center shadow-sm mb-6 border border-slate-800">
                <MessageSquare size={48} className="text-slate-300" />
              </div>
              <h3 className="text-xl font-bold text-slate-300 mb-2">No Transcript Selected</h3>
