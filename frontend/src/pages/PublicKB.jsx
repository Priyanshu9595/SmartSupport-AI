        ...
        <div className="text-center mb-12">
          ...
          <div className="relative max-w-xl mx-auto">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 rounded-full border border-white/50 shadow-md bg-slate-900/70 backdrop-blur-md focus:ring-2 focus:ring-blue-400 focus:border-blue-400 outline-none text-lg transition-all"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredArticles.length === 0 ? (
              <div className="text-center text-slate-500 py-8 bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white/50 shadow-md">
                No articles found matching "{search}".
              </div>
            ) : (
              filteredArticles.map(article => (
                <div key={article._id} className="bg-slate-900/60 backdrop-blur-xl rounded-2xl shadow-md border border-white/50 overflow-hidden transition-all duration-300 hover:shadow-lg">
                  <button 
                  ...
                  >
                    <span className="font-semibold text-slate-100 text-left">{article.question}</span>
                    {expandedId === article._id ? <ChevronUp className="text-slate-500" /> : <ChevronDown className="text-slate-500" />}
                  </button>
                  {expandedId === article._id && (
                    <div className="px-6 pb-4 pt-2 border-t border-slate-800 text-slate-300 leading-relaxed">
                      {article.answer}
                    </div>
                  )}
                </div>
