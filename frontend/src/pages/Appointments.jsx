    ...
    <div className="flex flex-col max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">Manage your team's upcoming video calls and client bookings.</p>
        </div>
      ...
      <div className="bg-slate-950/50 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 space-y-10 shadow-inner">
        ...
        <div>
          <div className="flex items-center mb-6">
            <h3 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">Upcoming Appointments</h3>
          ...
          {upcoming.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 border-dashed">
              <CalendarIcon size={48} className="mx-auto mb-4 text-slate-600 dark:text-slate-300" />
              <p className="text-lg font-bold text-slate-600 dark:text-slate-300">No upcoming appointments.</p>
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Your schedule is clear for now.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {upcoming.map(apt => {
                ...
                return (
                  <div key={apt._id} className={`bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm transition-all hover:shadow-md hover:-translate-y-1 ${isToday ? 'border-2 border-blue-400' : 'border border-slate-200 dark:border-slate-800'}`}>
                    
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-start">
                        <div className={`p-3 rounded-xl mr-4 flex-shrink-0 ${isToday ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
                           <Clock size={24} />
                        </div>
                        <div>
                          <div className={`font-extrabold text-xl tracking-tight mb-1 ${isToday ? 'text-blue-700' : 'text-slate-900 dark:text-white'}`}>
                    ...
                    <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-200 dark:border-slate-800 mb-5 text-[15px] space-y-3">
                      <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800/60 pb-3">
                         <span className="text-slate-500 dark:text-slate-400 font-medium text-sm">Service Type</span>
                         <span className="font-bold text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-800 text-sm">{apt.serviceType}</span>
                      </div>
                      
                      <div className="pt-1">
                        <div className="font-bold text-slate-900 dark:text-white mb-2 flex items-center"><UserIcon className="mr-2 text-slate-500 dark:text-slate-400" size={16}/> {apt.customerName}</div>
                        <div className="text-slate-500 dark:text-slate-400 text-sm flex items-center mb-1"><Mail size={14} className="mr-2"/> {apt.email}</div>
                        {apt.phone && <div className="text-slate-500 dark:text-slate-400 text-sm flex items-center"><Phone size={14} className="mr-2"/> {apt.phone}</div>}
                      </div>

                      {apt.message && (
                        <div className="pt-3 border-t border-slate-200 dark:border-slate-800/60">
                           <p className="text-sm text-slate-600 dark:text-slate-300 italic">"{apt.message}"</p>
                        </div>
                      )}
                      
                      <div className="pt-3 border-t border-slate-200 dark:border-slate-800/60 mt-4">
                        <div className="flex items-center font-bold text-slate-700 dark:text-slate-200 mb-2 text-xs uppercase tracking-wider">
                          <Video size={14} className="mr-2 text-slate-500 dark:text-slate-400" /> Google Meet Link
                        </div>
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-800 pt-5 flex flex-col space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <select
                          ...
                          className={`text-sm font-bold rounded-lg focus:ring-4 focus:ring-blue-100 outline-none block px-3 py-1.5 cursor-pointer transition-colors border-2 ${
                             ...
                             'bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-800 focus:border-slate-500'
                          }`}
                        >
                      ...
                      <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 pt-1">
                        ...
                        }} className="flex-1 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white hover:border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-950 px-3 py-2.5 rounded-xl text-sm font-bold transition flex items-center justify-center">
        ...
        {past.length > 0 && (
          <div>
            <div className="flex items-center mb-6">
              <h3 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">Past & Cancelled</h3>
              <div className="h-px bg-slate-200 flex-1 ml-6"></div>
            </div>
