...
const CustomerDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-white">
      <nav className="relative z-50 bg-slate-900/70 backdrop-blur-md border-b border-slate-700/50 px-4 md:px-8 py-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
        <Link to="/" className="text-xl font-bold text-blue-600">SupportFlow AI</Link>
        <div className="flex flex-wrap justify-center items-center gap-4 md:gap-6">
          <Link to="/" className="text-slate-300 hover:text-blue-600 font-medium text-sm">Home</Link>
          ...
          <div className="relative">
            <button onClick={() => setShowNotifications(!showNotifications)} className="text-slate-300 hover:text-blue-600 transition relative p-1 mt-1">
              <Bell size={20} />
              {unreadCount > 0 && (
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-80 md:w-96 bg-slate-900 rounded-2xl shadow-2xl border border-slate-800 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 origin-top-right">
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(notification => (
                      <div 
                        ...
                        className="p-4 border-b border-slate-800 hover:bg-slate-900 transition cursor-pointer flex flex-col gap-2 bg-slate-800/40"
                      >
      ...
      <main className="max-w-5xl mx-auto py-10 px-6">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-white mb-2">Hello, {user?.name}</h1>
          <p className="text-slate-500">Track the status of your support requests below.</p>
        </div>

        <div className="bg-slate-900/70 backdrop-blur-xl rounded-3xl shadow-lg border border-slate-700/50 overflow-hidden">
          <div className="p-6 border-b border-slate-700/50 bg-transparent flex items-center">
            <Ticket className="text-blue-600 mr-2" size={20} />
            <h2 className="text-lg font-bold text-white">My Ticket History</h2>
          </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {tickets.map(ticket => (
                    ...
                    >
                      <td className="py-4 px-6 font-medium text-white text-sm">{ticket.ticketId || 'N/A'}</td>
                      <td className="py-4 px-6 text-slate-200 text-sm max-w-xs truncate">{ticket.subject}</td>
        ...
        <div className="bg-slate-900/70 backdrop-blur-xl rounded-3xl shadow-lg border border-slate-700/50 overflow-hidden mt-8">
          <div className="p-6 border-b border-slate-700/50 bg-transparent flex items-center">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {appointments.map(appt => (
                    <tr key={appt._id} className="hover:bg-slate-800/60 transition-colors">
                      <td className="py-4 px-6 font-medium text-white text-sm">{appt.serviceType}</td>
                      <td className="py-4 px-6 text-slate-200 text-sm">{new Date(appt.dateTime).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' })}</td>
                      <td className="py-4 px-6 text-slate-200 text-sm">
                        {appt.meetingLink ? (
      ...
      {selectedTicket && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-700/60 w-full max-w-2xl max-h-[80vh] flex flex-col">
            
            {/* Modal Header */}
            <div className="p-6 border-b border-slate-700/50 flex justify-between items-center bg-transparent rounded-t-3xl">
              <div>
              ...
              <button onClick={() => setSelectedTicket(null)} className="text-slate-500 hover:text-slate-300 bg-slate-200/50 hover:bg-slate-200 p-2 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-transparent">
              ...
              <div className="mb-6 pb-6 border-b border-slate-700/50">
                <p className="text-sm font-semibold text-white mb-2">Original Request</p>
                <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-lg border border-slate-700/60 shadow-inner text-slate-200 text-sm whitespace-pre-wrap">
              ...
              {loadingMessages ? (
                ...
                <div className="text-center py-8 text-slate-500 text-sm italic bg-slate-900 border border-slate-800 rounded-lg shadow-sm">
                      ...
                      <div className={`max-w-[90%] p-4 text-sm ${
                        msg.senderType === 'Customer' 
                          ...
                          : msg.senderName === 'AI Support Bot'
                            ? 'bg-gradient-to-r from-violet-50 to-purple-50 border border-purple-100 text-white rounded-2xl rounded-tl-sm shadow-sm'
                            : 'bg-slate-900 border border-slate-700 text-white rounded-2xl rounded-tl-sm shadow-sm'
                      }`}>
                        <div className="flex items-center justify-between mb-2 opacity-90 text-xs font-bold text-slate-500">
                          <span className={`${msg.senderName === 'AI Support Bot' ? 'text-purple-700' : msg.senderType === 'Customer' ? 'text-blue-100' : 'text-slate-300'}`}>
                        ...
                        <p className={`whitespace-pre-wrap leading-relaxed ${msg.senderType === 'Customer' ? 'text-white' : 'text-slate-200'}`}>{msg.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
