    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-white">
      <main className="max-w-5xl mx-auto py-10 px-6">
        <div className="mb-8 flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-2">Hello, {user?.name}</h1>
            <p className="text-slate-500">Track the status of your support requests below.</p>
          </div>
          
          <div className="relative">
            <button onClick={() => setShowNotifications(!showNotifications)} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-xl shadow-sm text-slate-600 dark:text-slate-300 hover:text-blue-600 transition relative">
              <Bell size={20} />
              {unreadCount > 0 && (
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-72 sm:w-80 md:w-96 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 origin-top-right">
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(notification => (
                      <div 
                        ...
                        className="p-4 border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer flex flex-col gap-2"
                      >
        ...
        <div className="bg-slate-900/70 backdrop-blur-xl rounded-3xl shadow-lg border border-slate-300 dark:border-slate-700/50 overflow-hidden">
          <div className="p-6 border-b border-slate-300 dark:border-slate-700/50 bg-transparent flex items-center">
            <Ticket className="text-blue-600 mr-2" size={20} />
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">My Ticket History</h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-16 text-center">
              <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
                <Ticket className="text-slate-600 dark:text-slate-300" size={32} />
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-300 dark:border-slate-700 bg-slate-800/60">
                ...
                <tbody className="divide-y divide-slate-100">
                  {tickets.map(ticket => (
                    ...
                    >
                      <td className="py-4 px-6 font-medium text-slate-900 dark:text-white text-sm">{ticket.ticketId || 'N/A'}</td>
                      <td className="py-4 px-6 text-slate-700 dark:text-slate-200 text-sm max-w-xs truncate">{ticket.subject}</td>
                      <td className="py-4 px-6 text-slate-500 text-sm">{new Date(ticket.createdAt).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}</td>
        ...
        <div className="bg-slate-900/70 backdrop-blur-xl rounded-3xl shadow-lg border border-slate-300 dark:border-slate-700/50 overflow-hidden mt-8">
          <div className="p-6 border-b border-slate-300 dark:border-slate-700/50 bg-transparent flex items-center">
          ...
          {loadingAppointments ? (
            ...
            <div className="p-16 text-center">
              <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
                <Calendar className="text-slate-600 dark:text-slate-300" size={32} />
              </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {appointments.map(appt => (
                    <tr key={appt._id} className="hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors">
                      <td className="py-4 px-6 font-medium text-slate-900 dark:text-white text-sm">{appt.serviceType}</td>
                      <td className="py-4 px-6 text-slate-700 dark:text-slate-200 text-sm">{new Date(appt.dateTime).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' })}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-300 dark:border-slate-700/60 w-full max-w-2xl max-h-[90vh] md:max-h-[80vh] flex flex-col">
            ...
            <div className="p-6 border-b border-slate-300 dark:border-slate-700/50 flex justify-between items-center bg-transparent rounded-t-3xl">
              ...
              <button onClick={() => setSelectedTicket(null)} className="text-slate-500 hover:text-slate-600 dark:text-slate-300 bg-slate-200/50 hover:bg-slate-200 p-2 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-transparent">
              ...
              <div className="mb-6 pb-6 border-b border-slate-300 dark:border-slate-700/50">
                <p className="text-sm font-semibold text-slate-900 dark:text-white mb-2">Original Request</p>
                <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-lg border border-slate-300 dark:border-slate-700/60 shadow-inner text-slate-700 dark:text-slate-200 text-sm whitespace-pre-wrap">
                  {selectedTicket.description}
                </div>
              </div>

              {/* Conversation */}
              <p className="text-sm font-semibold text-slate-900 dark:text-white mb-4 flex items-center">
              ...
              {loadingMessages ? (
                ...
                <div className="text-center py-8 text-slate-500 text-sm italic bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm">
                    ...
                    <div key={index} className={`flex ${msg.senderType === 'Customer' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[90%] p-4 text-sm ${
                        msg.senderType === 'Customer' 
                          ? 'bg-blue-600 text-slate-900 dark:text-white rounded-2xl rounded-tr-sm shadow-sm' 
                          : msg.senderName === 'AI Support Bot'
                            ? 'bg-gradient-to-r from-violet-50 to-purple-50 border border-purple-100 text-slate-900 dark:text-white rounded-2xl rounded-tl-sm shadow-sm'
                            : 'bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-2xl rounded-tl-sm shadow-sm'
                      }`}>
                        <div className="flex items-center justify-between mb-2 opacity-90 text-xs font-bold text-slate-500">
                          <span className={`${msg.senderName === 'AI Support Bot' ? 'text-purple-700' : msg.senderType === 'Customer' ? 'text-blue-100' : 'text-slate-600 dark:text-slate-300'}`}>
                            {msg.senderName} {msg.senderName === 'AI Support Bot' && '✨'}
                          </span>
                          <span className={`ml-4 ${msg.senderType === 'Customer' ? 'text-blue-200' : 'text-slate-500'}`}>
                            {new Date(msg.createdAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata',hour: '2-digit', minute:'2-digit'})}
                          </span>
                        </div>
                        <p className={`whitespace-pre-wrap leading-relaxed ${msg.senderType === 'Customer' ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200'}`}>{msg.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
