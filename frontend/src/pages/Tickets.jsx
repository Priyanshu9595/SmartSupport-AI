...
const Tickets = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'Closed': return <span className="px-3 py-1 bg-slate-800 text-slate-200 text-xs font-bold rounded-lg border border-slate-800">Closed</span>;
  ...
  const getPriorityBadge = (priority) => {
    switch (priority) {
      ...
      default: return <span className="text-slate-300 font-bold text-xs bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-800 w-max">{priority}</span>;
    }
  };
      ...
      {viewMode === 'list' ? (
        <>
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-extrabold text-white tracking-tight">Helpdesk Tickets</h2>
              <p className="text-slate-400 mt-2">Manage and respond to customer issues and inquiries.</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-900 border border-slate-800 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none shadow-sm transition"
                value={searchQuery}
            ...
            <div className="flex gap-4">
              <div className="relative">
                ...
                <select 
                  className="pl-9 pr-8 py-3 bg-slate-900 border border-slate-800 rounded-xl outline-none shadow-sm font-semibold text-slate-200 appearance-none focus:ring-2 focus:ring-blue-500"
                  value={statusFilter}
          ...
          <div className="flex-1 overflow-hidden bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex flex-col">
            <div className="overflow-x-auto flex-1">
              <table className="min-w-full divide-y divide-slate-100">
                <thead className="bg-slate-950/80 sticky top-0 z-10 backdrop-blur-sm">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Ticket ID</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Customer Name</th>
                  ...
                  {tickets
                    ...
                    tickets
                      ...
                      <tr key={ticket._id} className="hover:bg-slate-950/80 transition-colors cursor-pointer group" onClick={() => handleSelectTicket(ticket)}>
                        <td className="px-6 py-4 whitespace-nowrap font-bold text-sm text-white">{ticket.ticketId || 'TCK-000'}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div>
                              <div className="text-sm font-bold text-white">{ticket.customerName}</div>
                              <div className="text-xs text-slate-400">{ticket.customerEmail}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-300 flex items-center h-full pt-6"><Bookmark size={14} className="mr-2 text-slate-400"/> {ticket.category || 'General'}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-400">{new Date(ticket.createdAt).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}</td>
        ...
        // Detail View
        <div className="flex-1 flex flex-col bg-slate-900 rounded-2xl shadow-sm border border-slate-800 overflow-hidden h-full">
          {/* Header */}
          <div className="bg-slate-900 p-6 border-b border-slate-800 flex items-center justify-between z-10">
            <div className="flex items-center">
              <button 
                onClick={() => setViewMode('list')}
                className="mr-5 p-2 text-slate-400 hover:text-slate-100 hover:bg-slate-800 rounded-xl transition-colors border border-transparent hover:border-slate-800"
              >
                <ChevronLeft size={24} />
              </button>
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  <h2 className="text-2xl font-extrabold text-white tracking-tight">{selectedTicket.subject}</h2>
                  <span className="text-xs font-bold bg-slate-800 border border-slate-800 text-slate-200 px-3 py-1 rounded-lg">{selectedTicket.ticketId}</span>
                </div>
                <div className="flex items-center text-sm text-slate-400 space-x-6 font-medium">
                  <span className="flex items-center"><User size={16} className="mr-2 text-slate-400"/> {selectedTicket.customerName}</span>
                  <span className="flex items-center"><Clock size={16} className="mr-2 text-slate-400"/> {new Date(selectedTicket.createdAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</span>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex flex-col text-right">
                ...
                <select 
                  ...
                  className="bg-slate-900 border-2 border-slate-800 text-slate-100 text-sm rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 block px-4 py-2 font-bold cursor-pointer outline-none transition-all hover:border-slate-700"
                >
                  <option value="Open">Open</option>
                  <option value="In Progress">In Progress</option>
                  <option value="Waiting for Customer">Waiting for Customer</option>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-2/3 flex flex-col border-r border-slate-800 bg-slate-950/50 relative">
              
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                {/* Initial Issue */}
                <div className="flex justify-start">
                  <div className="max-w-[85%] rounded-3xl rounded-tl-sm p-6 shadow-sm bg-slate-900 border border-slate-800 relative">
                    ...
                    <div className="flex justify-between items-center mb-4 pl-4">
                      <span className="text-sm font-bold text-white">{selectedTicket.customerName}</span>
                      <span className="text-xs font-semibold text-slate-400">{new Date(selectedTicket.createdAt).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata',hour: '2-digit', minute:'2-digit'})}</span>
                    </div>
                    <div className="text-[15px] text-slate-200 whitespace-pre-wrap leading-relaxed pl-4">{selectedTicket.description}</div>
                  </div>
                </div>
                ...
                {messages.map((msg, idx) => (
                  <div key={msg._id} className={`flex ${msg.senderType === 'Customer' ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[85%] rounded-3xl p-6 shadow-sm relative ${msg.senderType === 'Customer' ? 'rounded-tl-sm bg-slate-900 border border-slate-800' : 'rounded-tr-sm ' + (msg.isInternalNote ? 'bg-amber-50 border border-amber-200' : 'bg-blue-600 text-white border border-blue-700')}`}>
                      ...
                      <p className={`text-[15px] whitespace-pre-wrap leading-relaxed ${msg.senderType === 'Customer' ? 'pl-4 text-slate-200' : (msg.isInternalNote ? 'text-slate-200' : 'text-blue-50')}`}>
                        {msg.message}
                      </p>
                    </div>
                  </div>
              ...
              <div className="p-6 bg-slate-900 border-t border-slate-800">
                <form onSubmit={handleReply} className="flex flex-col space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-6 p-1 bg-slate-800 rounded-xl w-max">
                      <label className={`flex items-center px-4 py-2 text-sm font-bold rounded-lg cursor-pointer transition-all ${!isInternal ? 'bg-slate-900 shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-200'}`}>
                        <input type="radio" name="replyType" checked={!isInternal} onChange={() => setIsInternal(false)} className="sr-only" />
                        Public Reply
                      </label>
                      <label className={`flex items-center px-4 py-2 text-sm font-bold rounded-lg cursor-pointer transition-all ${isInternal ? 'bg-slate-900 shadow-sm text-amber-600' : 'text-slate-400 hover:text-slate-200'}`}>
                  ...
                  <div className="flex items-end space-x-4">
                    <textarea 
                      ...
                      className={`flex-1 p-4 border-2 rounded-2xl focus:ring-4 focus:outline-none resize-none shadow-sm transition-all text-[15px] ${isInternal ? 'border-amber-200 focus:ring-amber-100 focus:border-amber-400 bg-amber-50/30 placeholder-amber-400/70' : 'border-slate-800 focus:ring-blue-100 focus:border-blue-500 bg-slate-900 placeholder-slate-400'}`}
                      rows={3}
                    />
                    <button 
                      type="submit" 
            ...
            <div className="w-1/3 bg-slate-900 p-8 overflow-y-auto">
              ...
              <div className="flex items-center space-x-4 mb-8">
                ...
                <div>
                  <h4 className="font-bold text-white text-lg">{selectedTicket.customerName}</h4>
                  <a href={`mailto:${selectedTicket.customerEmail}`} className="font-medium text-slate-400 hover:text-blue-600 transition flex items-center mt-1 text-sm">
              ...
              <div className="space-y-6">
                <div>
                  <p className="text-xs font-bold text-slate-400 mb-2">Ticket Source</p>
                  <span className="px-3 py-1.5 bg-slate-800 text-slate-200 border border-slate-800 text-xs font-bold rounded-lg capitalize inline-flex items-center">
                ...
                {selectedTicket.keywordTags && selectedTicket.keywordTags.length > 0 && (
                  <div>
                    <p className="text-xs font-bold text-slate-400 mb-3">AI Auto-Tags</p>
